...
import { confirmEmail, confirmNick, getUserPassword, insertUserSql, getUserData, changeUserPassword, getUserId, getUserFromEmail, getUserTier, updateUserData,
    ...
    unscrapSentimentQuery, checkUserSentimentScrapStatusQuery, getAlarmInfo, alarmStatus, getAlarmStatus, getReadAlarms,getUnreadAlarms } from "./../models/user.sql.js";
...
export const getAlarmDao = async (userId) => {
    try {
        ...
        // 안 읽은 알림 가져오기
        const [unreadAlarms] = await pool.query(getUnreadAlarms, [userId]);

        // 읽은 알림 가져오기 (최대 10개)
        const [readAlarms] = await pool.query(getReadAlarms, [userId]);

        // 두 결과 합치기
        const alarms = [...unreadAlarms, ...readAlarms];

        // 최신 순으로 정렬
        alarms.sort((a, b) => b.created_at - a.created_at);
